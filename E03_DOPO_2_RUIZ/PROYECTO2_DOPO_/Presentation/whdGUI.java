...
public class whdGUI extends JFrame implements KeyListener {
    ...
    /**
     * Ejecuta un ciclo del juego: mueve enemigos, maquina, verifica colisiones.
     */
    ...
    /**
     * Maneja las teclas presionadas para movimiento de jugadores y pausa.
     * P1: WASD o flechas (4 direcciones) + Q, E, Z, C (diagonales).
     * P2: IJKL (solo en modo PvP).
     * P: pausar/reanudar.
     *
     * @param e evento de teclado
     */
    public void keyPressed(KeyEvent e) {
        if (game.isPaused() && e.getKeyCode() != KeyEvent.VK_P) {
            return;
        }

        int key = e.getKeyCode();

        if (key == KeyEvent.VK_UP    || key == KeyEvent.VK_W) { game.movePlayer(0, -1); }
        if (key == KeyEvent.VK_DOWN  || key == KeyEvent.VK_S) { game.movePlayer(0,  1); }
        if (key == KeyEvent.VK_LEFT  || key == KeyEvent.VK_A) { game.movePlayer(-1, 0); }
        if (key == KeyEvent.VK_RIGHT || key == KeyEvent.VK_D) { game.movePlayer( 1, 0); }

        if (key == KeyEvent.VK_Q) { game.movePlayer(-1, -1); }
        if (key == KeyEvent.VK_E) { game.movePlayer( 1, -1); }
        if (key == KeyEvent.VK_Z) { game.movePlayer(-1,  1); }
        if (key == KeyEvent.VK_C) { game.movePlayer( 1,  1); }

        if (game.getMode() == GameMode.PVP) {
            if (key == KeyEvent.VK_I) { game.movePlayer2(0, -1); }
            if (key == KeyEvent.VK_K) { game.movePlayer2(0,  1); }
            if (key == KeyEvent.VK_J) { game.movePlayer2(-1, 0); }
            if (key == KeyEvent.VK_L) { game.movePlayer2( 1, 0); }
        }
